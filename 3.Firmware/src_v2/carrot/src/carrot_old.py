...
while True:
    ...
    lower_carrot_b = np.array([73, 195, 0])
    upper_carrot_b = np.array([117, 255, 249])
    mask = cv2.inRange(hsv, lower_carrot, upper_carrot)
    mask = cv2.erode(mask, kernel, iterations=1)
    mask = cv2.dilate(mask, kernel, iterations=2)
    mask2 = cv2.inRange(hsv, lower_carrot_b, upper_carrot_b)
    ...
    _, contours_b, _ = cv2.findContours(mask2, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    #print(len(contours), len(contours_b))
    
    
    max_area, max_area_b = 0, 0
    sum_x , sum_y, sum_x_b , sum_y_b = 0, 0, 0, 0


    #try:
        
    if len(contours) == 0 and len(contours_b) == 0:
        max_area = area_threshold
        max_area_b = area_threshold
        mean_x = 320
        ...
        if len(contours_b) != 0:
            for contour in contours_b:
                ...
                if area > max_area_b:
                    max_area_b = area
                    max_contour_b = contour

            for j in range(len(max_contour_b)):
                sum_x_b = sum_x_b + max_contour_b[j][0][0]
                sum_y_b = sum_y_b + max_contour_b[j][0][1]
                
                
        #print(max_area_b, max_area)
        if max_area_b > max_area:
            mean_x  = sum_x_b/len(max_contour_b)
            mean_y = sum_y_b/len(max_contour_b)
            #print(max_area_b)
            if (area_threshold - max_area_b)>200:
                linear_vel_calc =  -100 + (+area_threshold**-0.5 - max_area_b**-0.5) * 5000
            ...
            if (area_threshold - max_area)>200:
                linear_vel_calc =  100 + (area_threshold**-0.5 + max_area**-0.5) * 3000
                if(linear_vel_calc > 150):
                    linear_vel_calc = 150
            else:
                linear_vel_calc = 0
    ...
    cv2.drawContours(frame, contours_b, -1, (0, 255, 0), 3)
    """except:
        print('i passed')
        pub_commands(0, 0)
        pass"""
