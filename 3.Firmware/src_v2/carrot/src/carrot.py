...
area_threshold, mid_x_axis = 3500, 320
...
while True:
    ...
    '''
    lower_carrot = np.array([0, 160, 173])
    upper_carrot = np.array([6, 202, 255])
    ...
    try:
        ...
        if (area_threshold - max_area)>0:
            linear_vel_calc =  120 - (area_threshold - max_area)**-0.5 * 1200
            if(linear_vel_calc > 120):
                linear_vel_calc = 120
        elif (area_threshold - max_area)<0:
            linear_vel_calc =  -120 + (-area_threshold + max_area)**-0.5 * 800
            if(linear_vel_calc < -120):
                linear_vel_calc = -120
        else:
            linear_vel_calc = 0
